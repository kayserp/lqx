    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "lqx" is a composite category enabling all Lqx-related debug output
                if(ptrCategory->count(std::string("lqx"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LqxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LqxCore
    // Mac: ~/Library/Application Support/LqxCore
    // Unix: ~/.lqxcore
#ifdef WIN32
    // Windows
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty lqx.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override lqx.conf
