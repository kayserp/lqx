...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 LQX irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on lqxcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in lqx.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("lqx.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in lqx.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Lqx features */
    { "lqx",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "lqx",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "lqx",               "gobject",                &gobject,                true,  {} },
    { "lqx",               "voteraw",                &voteraw,                true,  {} },
