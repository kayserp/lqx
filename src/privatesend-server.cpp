...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Lqx related functionality
...
/*
    ...
    allow endless transaction that would bloat Lqx and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Lqx specific functionality
