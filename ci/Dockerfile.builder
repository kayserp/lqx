...
# lqx_hash
RUN git clone https://github.com/vagnfedora/lqx_hash
RUN cd lqx_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} lqx
RUN useradd -u ${USER_ID} -g lqx -s /bin/bash -m -d /lqx lqx
...
RUN mkdir /lqx-src && \
  ...
  chown $USER_ID:$GROUP_ID /lqx-src && \
