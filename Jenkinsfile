...
// which are part of the Lqx repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="lqx-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /lqx-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "vagnfedora-lqx/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from vagnfedora-lqx/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'vagnfedora-lqx/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from vagnfedora-lqx/develop"
            }
          }

          if (hasCache) {
            sh "cd /lqx-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /lqx-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /lqx-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /lqx-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /lqx-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /lqx-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /lqx-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
